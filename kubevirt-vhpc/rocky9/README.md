# KubeVirt-Based HPC Cluster (Rocky Linux 9)

Virtual HPC cluster using KubeVirt on Kubernetes, running Rocky Linux 9 VMs. Creates a portable training environment with the ability to scale compute and storage nodes.

VM root disks are sourced from the official Rocky 9 GenericCloud qcow2, imported by CDI on first deploy. Override the image with `ROCKY_IMAGE_URL` in the Justfile.
